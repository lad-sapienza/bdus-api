...
const api = {
  ...
  /**
   * Opens tab and displays results of query
  ...
  query: {
    ...
    Export: function (obj_encoded, tb) {
      const html = '<select class="form-control export_format input-lg">' +
  ...
  link: {
            ...
            buttons: [
              {
                ...
                click: function () {
                  const aTrs = $('#list_' + div.find('div.id-holder').data('id')).dataTable().fnGetNodes();
                  let id_arr = [],
                    tb;
    ...
    show_userlinks: function (l_el) {
      const l_context = l_el.data('context');
      const l_tb = l_el.data('tb');
      const l_id = l_el.data('id');
      ...
      core.getHTML('userlinks_ctrl', 'showUserLinks', {
        ...
        $(l_el).find('span.userlink_add')
          .on('click', (e) => {
            ...
            const thistb = $(e.target).data('table');
   ...
   * @param opts  plain object with options
